const LATER_PLAYBACK_STORAGE_KEY = 'nio_play_later_v1';
...
function addLaterEpisode(items, episode) {
...
function removeLaterEpisode(items, episodeId) {
  return items.filter(item => String(item.id) !== String(episodeId));
}

function moveLaterEpisode(items, fromIndex, toIndex) {
...
function readLaterEpisodes(storage = globalThis.localStorage) {
  try {
...
function writeLaterEpisodes(items, storage = globalThis.localStorage) {
  try {
