...
export default function App({ initialCatalog = null }) {
  ...
  const startPlayback = useCallback((episode, visibleQueue = null) => {
    if (!episode) return;
    if (isLoopingEpisode(episode) && sleepTimerRef.current?.mode === 'episode-end') setSleepTimer(null);
    setAudioError(null);
    if (!episode.audioUrl) {
  ...
  const shareEpisode = useCallback(async episode => {
    ...
    if (navigator.clipboard?.writeText) {
      try { await navigator.clipboard.writeText(`${text} ${url}`); return 'copied'; } catch { return 'none'; }
    }
    return 'none';
  }, []);
    ...
    <main ref={appRef} className="app">
      ...
      <div className="app-content" inert={queuePresent ? true : undefined}>
        {!hasCatalog ? <div className="full-state">
          ...
          {route.screen === 'home' ? <HomeScreen catalog={catalog} daytimeEpisodes={daytimeResult?.episodes} player={player} stale={catalogState.stale} refreshing={catalogState.loading} catalogError={catalogState.error} onRetry={retryCatalog} onPlay={startPlayback} onPlayAll={playAll} onResume={resumePlayback} onTogglePlayback={togglePlayback} onOpenAlbums={openAlbums} onOpenFavorites={openFavorites} updatesCount={updatedAlbums.length} /> : null}
      ...
      <audio ref={audioRef} loop={loopingCurrentEpisode} playsInline preload="metadata" onLoadedMetadata={event => { const duration = event.currentTarget?.duration || 0; const { positionSeconds } = playerRef.current; if (!resumeSeekAppliedRef.current && canResume(positionSeconds, duration)) { try { event.currentTarget.currentTime = positionSeconds; } catch { /* media may not be ready */ } resumeSeekAppliedRef.current = true; } setPlayer(previous => ({ ...previous, durationSeconds: duration || previous.durationSeconds })); }} onTimeUpdate={event => { const position = event.currentTarget?.currentTime || 0; if (position > 0) playerRef.current = { ...playerRef.current, positionSeconds: position }; savePlayer(playerRef.current); if (!('mediaSession' in navigator) || typeof navigator.mediaSession.setPositionState !== 'function') return; try { navigator.mediaSession.setPositionState({ duration: event.currentTarget.duration || playerRef.current.durationSeconds || 0, position }); } catch { /* duration 未就绪时浏览器会抛错 */ } }} onPlay={() => { setIsPlaying(true); setAudioError(null); setPlayer(previous => ({ ...previous, isPlaying: true })); }} onPause={event => { if (event.currentTarget?.ended) return; const position = event.currentTarget?.currentTime || 0; setIsPlaying(false); setPlayer(previous => ({ ...previous, isPlaying: false, positionSeconds: position || previous.positionSeconds })); }} onError={() => setPlaybackFailure('音频加载失败，请检查网络后重试')} onEnded={handleEnded} />
      {miniPlayer}
