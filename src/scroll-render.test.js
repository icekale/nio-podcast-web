...
describe('mobile scroll rendering', () => {
  ...
  it('uses the calmer route and queue motion cadence', () => {
    expect(css).toContain('.route-view[data-route-motion="forward"] { animation: route-forward-in 320ms cubic-bezier(0.22, 1, 0.36, 1) both; }');
    expect(css).toContain('.route-view[data-route-motion="back"] { animation: route-back-in 320ms cubic-bezier(0.22, 1, 0.36, 1) both; }');
  ...
  it('defines the desktop shell, album grid, and queue drawer', () => {
    ...
    expect(css).toMatch(/\.album-results\.is-grid \.album-result > svg\s*\{[^}]*display:\s*none/);
    expect(css).toMatch(/\.album-screen \.episode-row:hover, \.later-picker \.episode-row:hover\s*\{[^}]*background:\s*var\(--surface-soft\)/);
