...
export function DesktopNav({ route, laterActive, updatesCount = 0, onHome, onSearch, onLater, onFavorites, showInstall, onInstall }) {
  return (
    <aside className="desktop-nav">
      ...
      <nav className="desktop-nav-links" aria-label="主导航">
        ...
        <button type="button" className="desktop-nav-link" aria-current={route.screen === 'favorites' ? 'page' : undefined} onClick={onFavorites}>专辑收藏{updatesCount > 0 && route.screen !== 'favorites' ? <span className="nav-update-dot" aria-hidden="true" /> : null}</button>
      </nav>
